...
bool Operator_Ext_Absorbing_BC::BuildExtension()
{
	...
	CSPropAbsorbingBC::ABCtype	boundaryType;
	...
	for(size_t n = 0 ; n < cs_props.size() ; ++n)
	{
		...
		boundaryType	= cs_ABS_props->GetAbsorbingBoundaryType();
		...
		for (size_t sheetIdx = 0 ; sheetIdx < cs_ABS_prims.size() ; ++sheetIdx)
		{
			...
			for (unsigned int dimIdx = 0 ; dimIdx < 3 ; dimIdx++)
			{
				...
				normDir = (Ncells[dimIdx] == 1) ? (dimIdx + 1) : normDir;
			}
			...
			for (unsigned int dimIdx = 0 ; dimIdx < 3 ; dimIdx++)
				dir[dimIdx] = (normDir + dimIdx - 1) % 3;

			// Add direction sign
			normDir *= (cs_ABS_props->GetNormalSignPositive() ? 1 : -1);

			// Currently not handling arbitrary phase velocity
			if (phaseVelocity == 0)
			{
				cerr 	<< "Operator_Ext_Absorbing_BC::BuildExtension(): Warning: Absorbing sheet Currently does not support per-material velocity. Setting to C0 "
						<< " ID: " << cs_ABS_prims.at(sheetIdx)->GetID() << " @ Property: " << cs_ABS_props->GetName() << endl;
			...
			unsigned int 	*temp_dir 		= new unsigned int[3];
			unsigned int	*temp_sheetX0 	= new unsigned int[3];
			unsigned int	*temp_sheetX1 	= new unsigned int[3];
			...
			if ((boundaryType == CSPropAbsorbingBC::MUR_1ST) || (boundaryType == CSPropAbsorbingBC::MUR_1ST_SA))
			{
			...
			if (boundaryType == CSPropAbsorbingBC::MUR_1ST_SA)
			{
	...
	m_numCells	= new unsigned int[m_numPrims];
	m_dir		= new unsigned int*[m_numPrims];
	m_sheetX0	= new unsigned int*[m_numPrims];
	m_sheetX1	= new unsigned int*[m_numPrims];
