	...
	m_K2_nyPP(op_ext->m_K2_nyPP)
{
	...
	for (unsigned int dimIdx = 0 ; dimIdx < 3 ; dimIdx++)
	{
		...
		m_dir[dimIdx] = m_Op_ABC->m_dir[dimIdx];
	}

	m_numLines[0] = m_Op_ABC->m_numLines[0];
	m_numLines[1] = m_Op_ABC->m_numLines[1];

	int normDir = m_Op_ABC->m_normDir;

	m_start_TS = 0;

	// Initialize shifted position for V
	m_pos_ny0_shift_V = m_posStart[m_dir[0]] + ((normDir >= 0) ? 1 : -1);

	// Initialize shifted position for I. Different for super-absorption
	m_pos_ny0_I = m_posStart[m_dir[0]] + ((normDir >= 0) ? 0 : -1);
	m_pos_ny0_shift_I = m_posStart[m_dir[0]] + ((normDir >= 0) ? 1 : -2);
...
void Engine_Ext_Absorbing_BC::DoPreVoltageUpdatesImpl(EngType* eng, int threadID)
{
	...
	pos[m_dir[0]] = m_posStart[m_dir[0]];
	pos_shift[m_dir[0]] = m_pos_ny0_shift_V;
	for (unsigned int i = m_threadStartLine.at(threadID) ; i < (m_threadStartLine.at(threadID) + m_linesPerThread.at(threadID)) ; i++)
	{
		...
		pos_shift[m_dir[1]] = pos[m_dir[1]] = m_posStart[m_dir[1]] + i;
		for (unsigned int j = 0; j < m_numLines[1]; j++)
		{
			pos_shift[m_dir[2]] = pos[m_dir[2]] = m_posStart[m_dir[2]] + j;

			// E(i + s,n) - K1*E(i,n)
			m_V_nyP [i][j] = eng->EngType::GetVolt(m_dir[1],pos_shift) - m_Op_ABC->m_K1_nyP [i][j] * eng->EngType::GetVolt(m_dir[1],pos);
			m_V_nyPP[i][j] = eng->EngType::GetVolt(m_dir[2],pos_shift) - m_Op_ABC->m_K1_nyPP[i][j] * eng->EngType::GetVolt(m_dir[2],pos);
		}
...
void Engine_Ext_Absorbing_BC::DoPostVoltageUpdatesImpl(EngType* eng, int threadID)
{
	...
	{
		// Store shifted location in this container
		pos_shift[m_dir[1]] = m_posStart[m_dir[1]] + i;
		for (unsigned int j = 0; j < m_numLines[1]; j++)
		{
			pos_shift[m_dir[2]] = m_posStart[m_dir[2]] + j;

			// E(i + s,n) - K1*E(i,n) + K1*E(i + s,n) =
			// E(i + s,n) + [K1*E(i + s,n) - K1*E(i,n)]
			m_V_nyP [i][j] += m_K1_nyP [i][j] * eng->EngType::GetVolt(m_dir[1],pos_shift);
			m_V_nyPP[i][j] += m_K1_nyPP[i][j] * eng->EngType::GetVolt(m_dir[2],pos_shift);
		}
...
void Engine_Ext_Absorbing_BC::Apply2VoltagesImpl(EngType* eng, int threadID)
{
	...
	{
		// Store shifted location in this container
		pos[m_dir[1]] = m_posStart[m_dir[1]] + i;
		for (unsigned int j = 0; j < m_numLines[1]; j++)
		{
			pos[m_dir[2]] = m_posStart[m_dir[2]] + j;

			// E(i,n + 1) = E(i + s,n) + [K1*E(i + s,n) - K1*E(i,n)]
			eng->EngType::SetVolt(m_dir[1],pos, m_V_nyP [i][j]);
			eng->EngType::SetVolt(m_dir[2],pos, m_V_nyPP[i][j]);
		}
...
void Engine_Ext_Absorbing_BC::DoPreCurrentUpdatesImpl(EngType* eng, int threadID)
{
	...
	pos[m_dir[0]] = m_pos_ny0_I;
	pos_shift[m_dir[0]] = m_pos_ny0_shift_I;
	for (unsigned int i = numLine_0 ; i < numLines_1 ; i++)
	{
		...
		for (unsigned int j = 0; j < (m_numLines[1] - 1); j++)
		{
			...
			// H(i + s,n) - K1*H(i,n)
			m_I_nyP [i][j] = eng->EngType::GetCurr(m_dir[1],pos_shift) - m_K1_nyP [i][j]*eng->EngType::GetCurr(m_dir[1],pos);
			m_I_nyPP[i][j] = eng->EngType::GetCurr(m_dir[2],pos_shift) - m_K1_nyPP[i][j]*eng->EngType::GetCurr(m_dir[2],pos);
		}
	}
...
void Engine_Ext_Absorbing_BC::DoPostCurrentUpdatesImpl(EngType* eng, int threadID)
{
	...
	for (unsigned int i = numLine_0 ; i < numLine_1 ; i++)
	{
		...
		for (unsigned int j = 0; j < (m_numLines[1] - 1); j++)
		{
			pos_shift[m_dir[2]] = m_posStart[m_dir[1]] + j;

			// H(i + s,n) + K1*[H(i,n + 1) - H(i,n)]
			m_I_nyP [i][j] += m_K1_nyP [i][j]*eng->EngType::GetCurr(m_dir[1],pos_shift);
			m_I_nyPP[i][j] += m_K1_nyPP[i][j]*eng->EngType::GetCurr(m_dir[2],pos_shift);
...
void Engine_Ext_Absorbing_BC::Apply2CurrentImpl(EngType* eng, int threadID)
{
	...
	for (unsigned int i = numLine_0 ; i < numLine_1 ; i++)
	{
		...
		for (unsigned int j = 0; j < (m_numLines[1] - 1); j++)
		{
			...
			eng->EngType::SetCurr(m_dir[1],pos, (m_I_nyP [i][j]*m_K2_nyP [i][j] + eng->EngType::GetCurr(m_dir[1],pos))/(m_K2_nyP [i][j] + 1.0));
			eng->EngType::SetCurr(m_dir[2],pos, (m_I_nyPP[i][j]*m_K2_nyPP[i][j] + eng->EngType::GetCurr(m_dir[2],pos))/(m_K2_nyPP[i][j] + 1.0));
