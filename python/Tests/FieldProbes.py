...
with HDF5Dump(os.path.join(Sim_Path, 'Et_n.h5')) as dE, HDF5Dump(os.path.join(Sim_Path, 'Dt_n.h5')) as dD, \
    ...
    t_idx = dE.GetNumTimesteps() // 4   # a timestep with the pulse inside the dump plane
...
with HDF5Dump(os.path.join(Sim_Path, 'Ht.h5')) as dH, HDF5Dump(os.path.join(Sim_Path, 'Bt.h5')) as dB:
    ...
    assert np.array_equal(dB.GetTimes(), dH.GetTimes()), 'FAIL: B-dump time axis differs from the H-dump (dual time)'
    t_idx = dH.GetNumTimesteps() // 4
