...
plt.figure()
plt.plot(freq / 1e6, np.real(Zin), "k-", linewidth=2, label=r"$\Re(Z_{in})$")
plt.grid()
plt.plot(freq / 1e6, np.imag(Zin), "r--", linewidth=2, label=r"$\Im(Z_{in})$")
...
plt.figure()
plt.plot(freq / 1e6, 20 * np.log10(abs(s11)), "k-", linewidth=2)
plt.grid()
...
E_CPRH = 20.0 * np.log10(
    np.abs(nf2ff_res.E_cprh[0]) / np.max(nf2ff_res.E_norm[0])
) + 10 * np.log10(nf2ff_res.Dmax[0])
E_CPLH = 20.0 * np.log10(
    np.abs(nf2ff_res.E_cplh[0]) / np.max(nf2ff_res.E_norm[0])
) + 10 * np.log10(nf2ff_res.Dmax[0])
...
plt.title('Frequency: {:.2f} GHz'.format(nf2ff_res.freq[0]/1e9))
plt.legend()
plt.show()
