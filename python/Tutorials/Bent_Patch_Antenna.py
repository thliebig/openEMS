...
start = [
    patch_radius + substrate_thickness,
    -substr_ang_width / 2,
    -substrate_length / 2,
]
stop = [patch_radius + substrate_thickness, +substr_ang_width / 2, substrate_length / 2]
...
port = FDTD.AddLumpedPort(
    1, feed_R, start, stop, "r", 1.0, priority=50, edges2grid="all"
)
...
if not len(idx) == 1:
    ...
    E_norm = 20.0 * np.log10(
        nf2ff_res_phi0.E_norm / np.max(nf2ff_res_phi0.E_norm)
    ) + 10.0 * np.log10(nf2ff_res_phi0.Dmax)
    ax.plot(
        np.deg2rad(theta),
        10 ** (np.squeeze(E_norm) / 20),
        linewidth=2,
        label="xz-plane",
    )
    ax.grid(True)
    ...
    E_norm = 20.0 * np.log10(
        nf2ff_res_theta90.E_norm / np.max(nf2ff_res_theta90.E_norm)
    ) + 10.0 * np.log10(nf2ff_res_theta90.Dmax)
    ax.plot(
        np.deg2rad(phi), 10 ** (np.squeeze(E_norm) / 20), linewidth=2, label="xy-plane"
    )
    ax.grid(True)
    ...
    print(
        "directivity:    Dmax = {:.1f} ({:.1f} dBi)".format(
            nf2ff_res_theta90.Dmax[0], 10 * np.log10(nf2ff_res_theta90.Dmax[0])
        )
    )
    print(
        "efficiency:   nu_rad = {:.1f} %".format(
            100 * nf2ff_res_theta90.Prad[0] / np.real(P_in[idx[0]])
        )
    )
