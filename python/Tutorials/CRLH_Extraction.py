...
class CRLH_Cells:
    ...
    def createCell(self, translate=[0, 0, 0]):
        ...
        mesh = mesh_hint_from_box(
            box, "x", metal_edge_res=self.edge_resolution, down_dir=False, mesh=mesh
        )
        mesh = mesh_hint_from_box(
            box, "y", metal_edge_res=self.edge_resolution, mesh=mesh
        )
        ...
        box = self.props["metal_top"].AddBox(start, stop, priority=10)
        mesh = mesh_hint_from_box(
            box, "x", metal_edge_res=self.edge_resolution, up_dir=False, mesh=mesh
        )
        ...
        mesh = mesh_hint_from_box(
            box, "x", metal_edge_res=self.edge_resolution, mesh=mesh
        )
        ...
        box = self.props["metal_bot"].AddBox(start, stop, priority=10)
        mesh = mesh_hint_from_box(
            box, "xy", metal_edge_res=self.edge_resolution, mesh=mesh
        )
...
if __name__ == "__main__":
    ...
    print(
        " Series tank: CL = {:.2f} pF,  LR = {:.2f} nH -> f_se = {:.2f} GHz ".format(
            CL * 1e12, LR * 1e9, fse * 1e-9
        )
    )
    print(
        " Shunt  tank: CR = {:.2f} pF,  LL = {:.2f} nH -> f_sh = {:.2f} GHz ".format(
            CR * 1e12, LL * 1e9, fsh * 1e-9
        )
    )
    ...
    wsh = 2 * np.pi * fsh
    beta_calc = np.real(
        np.arccos(1 - (w**2 - wse**2) * (w**2 - wsh**2) / (2 * w**2 / CR / LR))
    )
    ...
    plt.plot(
        abs(beta) * CRLH.LL * unit / np.pi,
        f * 1e-9,
        "k-",
        linewidth=2,
        label=r"$\beta_{CRLH,\ 1\ cell}$",
    )
    plt.grid()
    plt.plot(
        beta_calc / np.pi,
        ...
        "c--",
        linewidth=2,
        label=r"$\beta_{CRLH,\ \infty\ cells}$",
    )
    plt.plot(
        np.real(port[1].beta) * CRLH.LL * unit / np.pi,
        f * 1e-9,
        "g-",
        linewidth=2,
        label=r"$\beta_{MSL}$",
