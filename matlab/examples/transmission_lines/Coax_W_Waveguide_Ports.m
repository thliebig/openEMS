...
[CSX, port{2}] = AddWaveGuidePort(CSX, 15, 2, start_coax, stop_coax, 'z', '', '', kc , 0, 'E_WG_file', E_Mode_File, 'H_WG_file', H_Mode_File);
...
port = calcPort(port, Sim_Path, freq, 'RefImpedance', Zw);
...
figure;
plot(freq/1e9, 20*log10(abs([S11(:) S21(:)])),'linewidth',3);
set(gca,'fontsize',16);
grid on;
...
ylabel('S-Parameters (dB)','fontsize',20);

legHdl = legend('S11', 'S21');
set(legHdl,'fontsize',18);
