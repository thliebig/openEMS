...
bool HDF5_File_Reader::GetDataSetNames(hid_t &group, std::vector<std::string> &names)
{
	...
	hsize_t idx = 0;
#if H5_VERSION_GE(1,12,0)
	// version 2 explicitly: the unversioned H5Literate maps to the v1 API (H5L_info1_t)
	// on builds whose default API version is 1, e.g. Alpine
...
bool HDF5_File_Reader::CalcFDVectorData(std::vector<float> &frequencies, std::vector<ArrayLib::ArrayNIJK<std::complex<float>>*> &FD_data)
{
	...
	// so name by name the loop below would be quadratic
